...
The references for this summary are [@xiao_2023_introduction][@turner_2024_an][@phuong_2022_formal]
...
The goal of this part is to see if large language models understand or encode sentiment. We will follow the work presented in [@coenen_2019_visualizing], as well as [@hewitt_2019_a]. In this work, the authors have found a geometrical proof that transformer models encode word meanings, contextual meaning and syntax. However, we want to find a geometrical proof that transformers encode sentiment.

For this, we will proceed in two parts, that check two different ways in which the models could encode financial sentiment.
...
We have tried this over multiple indexes (Dow Jones, S&P, VIX, Russell 2000, Footsie and others), and they all yield the same results.

#### *Logistic Regressions against the MSCI World Index*

Since the exact returns of the MSCI cannot be determined with a linear regression, we perhaps thought that the model could more easily classify a specific quantile of returns. Meaning, even if the model can't be used to predict the *exact* return, it could potentially be used to predict if the returns are in the highest or lowest tranche. This could make sense since the news do not explain every single market variation, but they might explain a good part of the very drastic variations.

To check this, we can try to plot the embeddings and color by the quantiles, in order to see if there are apparent clusters ressembling the ones we had for the sentiment labels with movie reviews.

![Embeddings by quantiles](images/embeddings_quantiles.png){fig-align="center" width="50%"}

Here, we can see that there are no particular clusters in the data, besides the relationship we noticed earlier which maps embeddings to years. Here, we can see that during the 2008-2009 crisis, embeddings are more likely to belong to the 1st or to the last quantile, showing the important market volatility during this time.

Let's now show the confusion matrices for the results of our logistic regressions. Just like before, if we see that the model is overfit, we will try l2 penalization and then fitting the model on a reduction of the data using PCA and then UMAP.

Here are our results for this analysis.

![Logistic Regression, Raw Embeddings, No Penalty](images/log_reg_embed.png){fig-align="center" width="50%"}

![Logistic Regression, Raw Embeddings, L2 Penalty](images/log_reg_l2.png){fig-align="center" width="50%"}

![Logistic Regression, Reduced with PCA](images/log_reg_pca.png){fig-align="center" width="50%"}

![Logistic Regression, Reduced with UMAP](images/log_reg_reduced.png){fig-align="center" width="50%"}

\newpage

It is pretty much apparent that, just like with the linear regession, we can't seem to have a significant model which is not overfit during the training period.

If the reader is interesting in getting the specific metrics, we invite them to use the `get_logistic_reg()` function presented in the Annexe.

One thing to note is that the dimenstion reduction seemed much more useful when using a PCA to decompose the vectors. However, we can see that

\newpage

## Conclusion

In this analysis, we have tried to blend multiple areas of research together in order to explore the way that Large Language Models (LLMs) encode, or don't, events on financial markets. The idea was that, maybe, somewhere in the sentence embeddings of the model, there was some valuable and ineffable information about markets which could be exploited in order to create trading signals. We also wanted to explore the way that sentiment was or wasn't encoded in the model, in order to compare this result with the result on the link between financial headlines and markets. For this, we have used two datasets: one labelled datasetnthat was exclusively focused on sentiment, and another which was only a bank of dated financial headlines.

We have found meaningful clusters in the sentiment dataset, where human generated sentiment labels are found to replicate the natural unsupervised mathematical clusters. This points towards the direction that the model does encode sentiment and that this can be understood by the subspace that a certain embedding belongs to. This adds to existing research as it had only found a meaningful relationship between embeddings and specific word meaning, or between embeddings and sentence structure or syntax. However, we have also tried to match sentences together by their similarity, and in this way we have not been able to find a very significant relationship between the vectors, although the distribution of the cosine similarity and the euclidian distance did hint towards a typical difference between two similar vectors of opposite sentiment. This part of the analysis was generally inconclusive.

This means that although the model does place different sentence sentiments in different locations in the embedding space, this might not necessarily translate to the financial world. Indeed, our first dataset was comprised of movie reviews, which are generally composed of the same words and have a clear and distinct sentiment. However, in the financial world, things are much more complex. Some words may come up very often during certain years, and then not at all. There are a lot more numbers, and the sentiment is not always clear. In short, the data is much more noisy and harder to analyze. However, we thought that maybe all of this noise might give us additional information which could help the model in understanding market phenomenons. Therefore, we tried to find a regression which could help us to predict market moves from sentence embeddings.

However, the results of such regressions were unsuccessful. It seems that there is simply too much noise in the data to really get a clear "answer" from the model as to whether or not it is bull or bear on the market. This might be for a variety of reasons. For instance, not having data that related to hours and minutes forced us to cram multiple sentences together and generate one embedding per day. This also limited our sample size since we were limited by the context size of the model. This means that there will be even more noise in our data. Secondly, we have a dataset which is very hard to compare to a specific index, since it contains worldwide news. The ideal dataset for this analysis would be a lot more granular. This means that although we couldn't make the regressions work here, this does not exclude the possibility that LLMs encode information which could be useful to traders.

\newpage

## Annexe

### [Generating embeddings]{.underline}
...
def import_sentences( dated: bool = False, consensus: str = "All", reduce_frame: bool = True) -> pl.LazyFrame:
    ...
    if dated:
        sentences = (
            pl.read_csv(
                r"C:\Users\faune\Downloads\lab1\lab1\data\dow_jones_news.csv", separator=';'
            )
            ...
            .with_columns(
                pl.col('label').replace({0:'negative', 1:'positive'}), 
        ...
        if reduce_frame:
            sentences = (
                ...
                .collect()
                .filter(
                    pl.col('date').dt.year().gt(2010), 
                    pl.col('sentences').str.contains(pattern = '[a-zA-Z]+')
                )
                .group_by('date')
...
def _get_embeddings_by_chunks(data: list, chunk_size: int, api_key: str = API_KEY): 
    # mostly COPIED FROM https://docs.mistral.ai/capabilities/embeddings/
    client = MistralClient(api_key=api_key)
...
def get_embeddings(dated: bool = False, consensus: str = 'All', reduce_frame: bool = True) -> pl.LazyFrame:
    ...
    Args:
        dated (bool, optional): gets the sentences from the dated dataset (origin: lab1). Defaults to False.
        consensus (str, optional): consensus for the labels on the 2nd dataset (undated, used to train finBERT).
        Defaults to "All".
        Can be either "All", "75", "66" or "50". Don't forget to use a string data type even if you use the 
        numbered options.
        reduce_frame (bool, optional): reduces size of the frame from 1m + to ~20 000. Defaults to True.
        (only data after 2011 + only euro characters + divides the nb of sentences per day by 40)

    Returns:
        pl.LazyFrame: lazyframe with 'sentences', 'label' and 'embeddings' columns (and also 'date' if dated = 
        True) 
    """    
    ...
    return (
        ...
        .with_columns(
            pl.Series(
                name= 'embeddings', 
                values= _get_embeddings_by_chunks(
                    frame.select('sentences').collect().to_series().to_list(),
                    50
                )
            )
        )
...
merged = (
    positive_quantile_df
    .merge(
        negative_quantile_df[['negative_idx','sentence','processed']]
        .rename(
            {'sentence':'neg_sentence','processed':'neg_processed'},axis=1),
                            how='left',
...
with_metrics = (
    matched_sentences.unique().with_columns(
         matched_sentences
        .with_columns(
            pl.col('embeddings').list.to_array(1024),
            pl.col('embeddings_neg').list.to_array(1024)
        )
        .map_rows(
            function=lambda t : tuple(cosine_similarity(X=np.array(t[4]).reshape(1, -1), 
                                                        Y=np.array(t[5]).reshape(1, -1)).tolist()))
    )
    .explode('column_0')
    .sort('column_0')
    ...
    .with_columns(
        matched_sentences
        .with_columns(pl.col('embeddings').list.to_array(1024),pl.col('embeddings_neg').list.to_array(1024))
    ...
    .with_columns(
        ...
        .with_columns(pl.col('embeddings').list.to_array(1024),pl.col('embeddings_neg').list.to_array(1024))
        .map_rows(
            function=lambda t : tuple(
                euclidean_distances(X=np.array(t[4]).reshape(1, -1),
                                                          Y=np.array(t[5]).reshape(1, -1)).tolist()))
    )
    .explode('column_0')
...
### [Pre-process for regressions]{.underline}
...
frame = frame.sort('date').set_sorted('date').with_columns(pl.col('close').rolling_std(by='date', window_size = '1w').alias('std')).drop_nulls()
...
### [Logistic Regressions]{.underline}
...
Everything else (executable notebooks, etc) can be found on the github for this masters thesis : [Github Thesis](https://github.com/faunebl/thesis)
